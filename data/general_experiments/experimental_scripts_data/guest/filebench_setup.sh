...
do
   name="c$i"
   echo 128M > /sys/fs/cgroup/memory/lxc/$name/memory.limit_in_bytes
   echo 5G > /sys/fs/cgroup/memory/lxc/$name/memory.memsw.limit_in_bytes

   echo 500 > /sys/fs/cgroup/blkio/lxc/$name/blkio.weight
...
echo 2-5 >  /sys/fs/cgroup/cpuset/lxc/c2/cpuset.cpus
...
sudo -u root sshpass -p 12345 ssh 10.129.78.110 "cd /home/deba/prashanth/experiment/; helper_scripts/filebench_logger.sh $poolid_1 $poolid_2 $poolid_3 $poolid_4 $dir $exp" &

# Start logger
bash helper_scripts/filebench_logger.sh $dir $exp &
