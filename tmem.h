...
/*
 * RB-Tree that keeps track of all existing objects for a pool
 */
...
extern int tmem_evict_page(void *, uint32_t, void *);
void lock_all_tmem_hb(struct tmem_pool*);
void unlock_all_tmem_hb(struct tmem_pool*);
extern int tmem_object_flush_unlocked(struct tmem_pool*, struct tmem_hashbucket *, struct tmem_obj*, int, bool *); 
extern int tmem_check_lock_obj(struct tmem_hashbucket *, struct tmem_obj*);
